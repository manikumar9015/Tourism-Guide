            ...
            <div class="booking-list">
                ...
                if (!empty($userBookings)) {
                    foreach ($userBookings as $booking) {
                        ...
                        echo '<button class="remove-btn" onclick="removeBooking(' . $booking['id'] . ', ' . $_SESSION['user_id'] . ')">Remove Booking</button>';
                        echo '</div>';
                    }
                } else {
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    if (xhr.responseText === "success") {
                        ...
                        // Update the booking list without reloading the page
                        var bookingList = document.querySelector('.booking-list');
                        var newBooking = document.createElement('div');
                        newBooking.classList.add('booking');
                        newBooking.innerHTML = '<span><b>Restaurant:</b> ' + form.elements.restaurant_id.options[form.elements.restaurant_id.selectedIndex].text + '</span>' +
                                              '<span><b>Date:</b> ' + form.elements.booking_date.value + '</span>' +
                                              '<span><b>Time:</b> ' + form.elements.booking_time.value + '</span>' +
                                              '<span><b>Guests:</b> ' + form.elements.num_guests.value + '</span>' +
                                              '<span><b>Total Cost:</b> $' + (form.elements.num_guests.value * <?php echo getRestaurantPrice('{restaurant_id}', $conn); ?>) + '</span>' +
                                              '<button class="remove-btn" onclick="removeBooking(' + $booking['id'] + ', ' + <?php echo $_SESSION['user_id']; ?> + ')">Remove Booking</button>';
